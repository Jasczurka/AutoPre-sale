...
public class LoginUseCase
{
    ...
    public async Task<Result<LoginResponse, LoginError>> Execute(LoginRequest request)
    {
        ...
        if (user.PasswordHash == null || user.PasswordHash.Length == 0 || string.IsNullOrEmpty(user.Salt))
        {
            return Result<LoginResponse, LoginError>.Fail(LoginError.InvalidCredentials);
        }
        ...
        // Обновляем пользователя для сохранения refresh token
        var userToUpdate = await _userRepository.GetUserById(user.Id.ToString());
        if (userToUpdate != null)
        {
            userToUpdate.RefreshTokens.Add(issued.Stored);
